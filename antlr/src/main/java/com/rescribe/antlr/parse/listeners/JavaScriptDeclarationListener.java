...
public class JavaScriptDeclarationListener extends JavaScriptParserBaseListener
    implements CustomListener {
  File file;
  BufferedTokenStream tokens;
  Stack<Parent> parents = new Stack<>();

  public JavaScriptDeclarationListener(
  ...
  public void enterAnoymousFunctionDecl(JavaScriptParser.AnoymousFunctionDeclContext ctx) {
    ...
    if (ctx.getChildCount() == 6) {
      int length =
          ctx.children.get(4).getText().length() > functionNameChars
              ? functionNameChars
              : ctx.children.get(4).getText().length();
